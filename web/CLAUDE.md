...
rail when there is one and the account when there is not. `sw.js` is v19 for
remarks and bills on the entry form.
...
- **`/offline` is `force-dynamic`** though it reads nothing, because every
  ...
  with none. The worker (`public/sw.js`, `VERSION = 'v19'`) fetches it once at
