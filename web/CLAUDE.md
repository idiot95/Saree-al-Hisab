...
Icons are generated by `npm run icons` — raw pixels and a hand-written PNG
encoder, because adding an image library to a finance app so it can draw three
lines is a poor trade. The maskable variant keeps the mark inside the middle
80%, or Android's circular crop takes the ends off the rules. iOS ignores the
