...
export async function signInWithPassword(_prev: AuthResult, fd: FormData): Promise<AuthResult> {
  ...
  if (!email || !password) return { error: 'Fill in both boxes.' };
  ...
  if (user && isLocked(user)) {
    return { error: `Too many tries. Try again ${waitFor(user.locked_until)}.` };
  }
  ...
  try {
    ...
    if (e instanceof AuthError) return { error: 'That address and password do not match.' };
