...
export default function NewSchedule({ methods, categories, startOpen = false }: {
  ...
  return (
    <form action={act} className="el" style={{
        ...
        <div style={{ display: 'flex', gap: 7 }}>
          {[['MONTHLY', 'Every month'], ['YEARLY', 'Once a year']].map(([id, label], i) => (
            <label key={id} style={{
              ...
              gap: 7, borderRadius: 12, cursor: 'pointer', fontSize: 14, fontWeight: 600,
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        {freq === 'YEARLY' && (
          <label style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 6 }}>
            <span style={{ fontSize: 12.5, fontWeight: 600, color: 'var(--c-meta)' }}>Month</span>
        ...
        <label style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 6 }}>
          <span style={{ fontSize: 12.5, fontWeight: 600, color: 'var(--c-meta)' }}>Day</span>
      ...
      <label style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        <span style={{ fontSize: 12.5, fontWeight: 600, color: 'var(--c-meta)' }}>Category</span>
