...
/* Swipe a row to the left and its actions come out from under it.
   ...
   Only one row is open at a time, a tap anywhere else closes it, and a drag
   never turns into a tap — so a row that is also a link is not followed by
...
export default function SwipeRow({ actions, commit = true, children }: {
  ...
  // A tap anywhere outside closes an open row.
  useEffect(() => {
    if (!open) return;
  ...
  const down = (e: React.PointerEvent) => {
    ...
    s.current = { id: e.pointerId, sx: e.clientX, sy: e.clientY, x0: x, x, axis: '', moved: false, w, armed: false };
  };
