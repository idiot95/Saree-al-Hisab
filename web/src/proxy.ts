...
/* An optimistic check only. Next's own guidance is explicit that Proxy is not
   ...
   /join is excluded on purpose: an invitation has to open for someone who is
   not signed in yet, which is the entire point of it. */
export const config = {
  matcher: ['/((?!api|_next/static|_next/image|icons|manifest.webmanifest|favicon.ico|signin|join).*)'],
};
